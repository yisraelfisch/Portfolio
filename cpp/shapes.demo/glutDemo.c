...
/* ellipse data */
static Point ellipsePos = { 500, 500};
static int ellipseStep = 5;
...
static void DrawFunction()
{
    ...
    DrawEllipse(COLOR_YELLOW, ellipsePos.m_x, ellipsePos.m_y, 200, 80, 100);
}
...
static int TimerFunction()
{
    ...
    rectPos.m_y += rectStep; /* step the rectangle vertically */
    if (rectPos.m_y > 800 || rectPos.m_y < 100)
        rectStep = -rectStep;

    ellipsePos.m_x += ellipseStep; /* step the ellipse horizontally */
    if (ellipsePos.m_x > 780 || ellipsePos.m_x < 220)
        ellipseStep = -ellipseStep;

    Revolve(+3.14/100, &circPivot, &circCenter); /* revolve the circle */
