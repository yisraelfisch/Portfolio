...
#include <sys/types.h>
  ...
  struct buse_operations {
    int (*read)(void *buf, u_int32_t len, u_int64_t offset, void *userdata);/*read from userdata to buf*/
    int (*write)(const void *buf, u_int32_t len, u_int64_t offset, void *userdata);/*write from buf to userdata*/
    ...
    u_int64_t size; /*size = blksize * size_blocks*/
    u_int32_t blksize;
