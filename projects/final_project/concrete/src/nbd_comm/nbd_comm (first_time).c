...
NbdRequest *NbdRequestRead(int sock)
{
    ...
    nbd_message_t *new_message = malloc(sizeof(nbd_message_t));
    read_all(sock, (char *)&request, sizeof(request));

    if(!(new_message = (nbd_message_t *)malloc(offsetof(nbd_message_t, buffer) + htonl(request.len))))
    {
        perror("malloc");
        return NULL;
    }
    ...
    new_message->nbd_req.reqType = htonl(request.type);
    new_message->nbd_req.dataLen = htonl(request.len);
